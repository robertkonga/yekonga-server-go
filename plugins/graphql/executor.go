...
// 		if !exist {
// 			fieldPath := p.Path.WithKey(responseName)
// 			result, state := resolveField(p.ExecutionContext, p.ParentType, p.Source, fieldASTs, fieldPath)
// 			if state.hasNoFieldDefs {
// 				continue
// 			} else {
// 				resolved = result
...
// executeSubFields concurrently processes fields and returns a map
func executeSubFields(p executeFieldsParams) map[string]interface{} {
	if p.Source == nil {
		p.Source = map[string]interface{}{}
	}
	...
	for i := 0; i < numWorkers; i++ {
		...
		go func() {
			...
			for task := range taskQueue {
				fieldPath := p.Path.WithKey(task.responseName)
				resolved, state := resolveField(p.ExecutionContext, p.ParentType, p.Source, task.fieldASTs, fieldPath)
				if !state.hasNoFieldDefs {
					resultChan <- struct {
						responseName string
						resolved     interface{}
					}{task.responseName, resolved}
				}
			}
		}()
	}
	...
	close(taskQueue) // No more tasks to send

	// Collect results in the main goroutine
	go func() {
		wg.Wait()         // Wait for all workers to finish
		close(resultChan) // Close result channel after all workers finish
	}()

	// Read results into finalResults
...
func DefaultResolveFn(p ResolveParams) (interface{}, error) {
	...
	if sourceVal.Type().Kind() == reflect.Struct {
		for i := 0; i < sourceVal.NumField(); i++ {
			...
			// fmt.Printf("%v => %v", valueField, typeField)
